...
Popup {
    ...
    function bindModel(){
        if (recordId > 0){
            backend.requestEmployeeInfo(recordId);
        }
        else{
    ...
    function saveModel(){
        ...
        delay(200, function(){
            backend.saveEmployee(JSON.stringify({
                id: recordId,
    ...
    function deleteModel(){
        waitingIcon.visible = true;
        delay(200, function(){
            backend.deleteEmployee(recordId);
        });
    }
    ...
    Connections {
        ...
        function onGetEmployeeInfo(employeeInfo){
            const data = JSON.parse(employeeInfo);
            if (data){
        ...
        function onSaveEmployeeFinished(saveResult){
            waitingIcon.visible = false;
            var resultData = JSON.parse(saveResult);
            if (resultData){
                if (resultData.Result){
                    recordId = resultData.RecordId
                    backend.broadcastEmployeeListRefresh();
                }
            }
        }

        function onDeleteEmployeeFinished(saveResult){
            waitingIcon.visible = false;
            var resultData = JSON.parse(saveResult);
            if (resultData){
                if (resultData.Result){
